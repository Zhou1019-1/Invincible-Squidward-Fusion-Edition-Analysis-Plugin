...
class ConfigManager:
    ...
    def _parse_config(self, config: dict):
        ...
        self.download = DownloadConfig(
            ...
            oversize_as_file=self._parse_bool(
                download_raw.get("oversize_as_file", False),
                False,
                "download.oversize_as_file",
            ),
        )
        ...
        self.bilibili = BilibiliEnhancedConfig(
            ...
            cdn_mode=self._parse_non_negative_int(bili.get("cdn_mode", 0), 0),
            video_codec=str(bili.get("video_codec", "auto") or "auto").strip(),
            file_size_limit_mb=self._parse_non_negative_int(
                bili.get("file_size_limit_mb", 0), 0
            ),
            show_online=self._parse_bool(
                bili.get("show_online", False),
                False,
                "bilibili_enhanced.show_online",
            ),
            ai_summary=self._parse_bool(
                bili.get("ai_summary", False),
                False,
                "bilibili_enhanced.ai_summary",
            ),
            live_parse=self._parse_bool(
                bili.get("live_parse", False),
                False,
                "bilibili_enhanced.live_parse",
            ),
            live_clip_seconds=self._parse_non_negative_int(
                bili.get("live_clip_seconds", 0), 0
            ),
        )

        # --- douyin_enhanced ---
        douyin_raw = config.get("douyin_enhanced", {})
        if not isinstance(douyin_raw, dict):
            douyin_raw = {}
        self.douyin = DouyinEnhancedConfig(
            live_parse=self._parse_bool(
                douyin_raw.get("live_parse", False),
                False,
                "douyin_enhanced.live_parse",
            ),
            live_clip_seconds=self._parse_non_negative_int(
                douyin_raw.get("live_clip_seconds", 0), 0
            ),
            send_bgm=self._parse_bool(
                douyin_raw.get("send_bgm", False),
                False,
                "douyin_enhanced.send_bgm",
            ),
        )
    ...
    def create_parsers(self) -> List:
        ...
        douyin_hc = self._effective_hot_comment_count(
            self.message.hot_comments.douyin,
            "douyin",
        )
        ...
        if self._enable_bilibili:
            self.bilibili_parser = BilibiliParser(
                ...
                cdn_mode=self.bilibili.cdn_mode,
                video_codec=self.bilibili.video_codec,
                file_size_limit_mb=self.bilibili.file_size_limit_mb,
                show_online=self.bilibili.show_online,
                ai_summary=self.bilibili.ai_summary,
                live_parse=self.bilibili.live_parse,
                live_clip_seconds=self.bilibili.live_clip_seconds,
            )
            parsers.append(self.bilibili_parser)
        if self._enable_douyin:
            parsers.append(
                DouyinParser(
                    hot_comment_count=douyin_hc,
                    live_parse=self.douyin.live_parse,
                    live_clip_seconds=self.douyin.live_clip_seconds,
                    send_bgm=self.douyin.send_bgm,
                )
            )
