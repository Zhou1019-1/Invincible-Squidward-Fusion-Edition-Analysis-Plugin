...
def build_media_nodes(
    ...
    for idx, url_list in enumerate(video_urls):
        ...
        if mode == "skip":
            ...
            continue
        if mode == "file":
            # 超限转文件模式：以文件消息发送本地视频（移植自 rconsole-plugin）
            file_path_value = (
                file_paths[file_idx] if file_idx < len(file_paths) else None
            )
            if (
                File is not None
                and file_path_value
                and os.path.exists(file_path_value)
            ):
                try:
                    nodes.append(
                        File(
                            name=os.path.basename(file_path_value),
                            file=file_path_value,
                        )
                    )
                except Exception as e:
                    logger.warning(f"构建超限视频文件节点失败: {e}")
            else:
                _mark_media_failure(
                    metadata, "video", idx, "超限视频文件不存在或组件不可用"
                )
            file_idx += 1
            continue
...
def build_music_node(metadata: Dict[str, Any]) -> Optional[Any]:
    """构建BGM语音节点（移植自 rconsole-plugin 的音乐附带能力）。

    Args:
        metadata: 元数据字典，包含可选的 music_url 字段

    Returns:
        Record 语音节点，无BGM或组件不可用时为None
    """
    if Record is None:
        return None
    music_url = str(metadata.get("music_url", "") or "").strip()
    if not music_url:
        return None
    try:
        return Record.fromURL(music_url)
    except Exception as e:
        logger.warning(f"构建BGM语音节点失败: {e}")
        return None
...
def _build_node_parts_for_link(
    ...
    if effective_rich_media:
        music_node = build_music_node(metadata)
