...
| 屏蔽用户解析 | 管理员发送 `#禁用解析@某用户`（或 `#禁用解析 QQ号`）永久屏蔽该用户，其消息不再触发解析；`#恢复解析@某用户` 解除。名单持久化保存，重启不丢失 | 无需配置，需先设置 `permissions.admin_id` |
...
- **权限控制**：管理员 ID + 用户/群组黑白名单，优先级：管理员 > 个人白名单 > 个人黑名单 > 群组白名单 > 群组黑名单；管理员还可用 `#禁用解析@某用户` 指令永久屏蔽指定用户（`#恢复解析` 解除）。
