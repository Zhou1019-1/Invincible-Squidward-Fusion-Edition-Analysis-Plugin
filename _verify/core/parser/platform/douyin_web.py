...
DOUYIN_LIVE_ENTER_API = "https://live.douyin.com/webcast/room/web/enter/"
...
class DouyinWebClient:
    ...
    async def _signed_get(
        ...
        """发起一次 a_bogus 签名 + ttwid 会话的 GET 请求，失败返回None。"""
        ...
        url = f"{api}?{param_string}&a_bogus={signature}"
        headers = {
            "User-Agent": DOUYIN_WEB_USER_AGENT,
            "Referer": referer or DOUYIN_REFERER,
            "Accept": "application/json, text/plain, */*",
    ...
    async def fetch_comments(
        ...
        """获取作品评论列表（移植自 rconsole-plugin），失败时返回空列表。
        ...
        Returns:
            标准化评论字典列表（username/uid/likes/text），按点赞数降序
        """
        if count <= 0:
            return []
        ...
        for comment in data.get("comments") or []:
            ...
            if not text:
                continue
            try:
                likes = int(comment.get("digg_count", 0) or 0)
            except (TypeError, ValueError):
                likes = 0
            comments.append(
                {
        ...
        comments.sort(key=lambda item: item.get("likes", 0), reverse=True)
    ...
    async def fetch_live_room(
        ...
        """获取抖音直播间信息（移植自 rconsole-plugin），失败时返回None。"""
        data = await self._signed_get(
            session,
            DOUYIN_LIVE_ENTER_API,
            {
                "aid": "6383",
                "app_name": "douyin_web",
                "live_id": "1",
