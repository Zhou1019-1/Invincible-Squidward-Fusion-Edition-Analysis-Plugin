...
class DouyinParser(ShortVideoParserMixin, BaseVideoParser):
    ...
    @staticmethod
    def _is_douyin_live_url(url: str) -> bool:
        """判断是否为抖音直播间链接（含房间号）。"""
        try:
            parsed = urlparse(url)
        ...
        return host == "live.douyin.com" and bool(
            re.search(r"^/(\d+)(?:/|$)", parsed.path or "")
        )

    @staticmethod
    def _extract_douyin_item_id(*urls: str) -> str:
        """从URL序列中提取作品ID（video/note/slides路径或19位数字）。"""
        for url in urls:
            for pattern in (r"/(?:video|note|slides)/(\d+)", r"(\d{19})"):
    ...
    def can_parse(self, url: str) -> bool:
        ...
        if self._is_douyin_live_url(url):
            if self.live_parse:
                logger.debug(f"[{self.name}] can_parse: 匹配抖音直播链接 {url}")
                return True
            logger.debug(f"[{self.name}] can_parse: 跳过抖音直播链接 {url}")
            return False
    ...
    def extract_links(self, text: str) -> List[str]:
        ...
        if self.live_parse:
            live_pattern = r'https?://live\.douyin\.com/(\d+)[^\s<>"\'()]*'
            for match in re.finditer(live_pattern, text, re.IGNORECASE):
                matched_url = self._clean_extracted_url(match.group(0))
                if not matched_url:
                    continue
                room_id = match.group(1)
                key = f"douyin:live:{room_id}"
                if key in seen_keys or matched_url in seen_urls:
                    continue
                seen_keys.add(key)
                seen_urls.add(matched_url)
                result_links.append(f"https://live.douyin.com/{room_id}")
    ...
    def _build_douyin_result_from_item(
        ...
        # 提取BGM（移植自 rconsole-plugin 的音乐附带能力）
        music_url = ""
        music_title = ""
        ...
        if isinstance(music_info, dict):
            ...
            if isinstance(play_url, dict):
                candidates = (
                    play_url.get("url_list") or play_url.get("urlList") or []
                )
                music_url = next(
                    (
                        u
            ...
            music_title = str(music_info.get("title", "") or "").strip()
    ...
    @staticmethod
    def _extract_live_flv_url(stream_url_info: Dict[str, Any]) -> str:
        """从直播间流信息中提取FLV地址（优先高画质档位）。"""
        pull = stream_url_info.get("flv_pull_url")
        if isinstance(pull, dict):
            for key in ("FULL_HD1", "HD1", "SD1", "SD2"):
                url = pull.get(key)
                if isinstance(url, str) and url.startswith("http"):
                    return url
            for url in pull.values():
                if isinstance(url, str) and url.startswith("http"):
                    return url
        return ""

    async def _parse_douyin_live(
        self, session: aiohttp.ClientSession, url: str
    ) -> Dict[str, Any]:
        """解析抖音直播间信息与可选直播片段（移植自 rconsole-plugin）。

        Raises:
            SkipParse: 直播间信息获取失败时回退为跳过（保持原有行为）
        """
        match = re.search(r"live\.douyin\.com/(\d+)", url)
        if not match:
            raise RuntimeError(f"无法解析抖音直播间链接: {url}")
        room_id = match.group(1)
        data = await self.web_client.fetch_live_room(session, room_id)
        room = None
        if data:
            room_list = (data.get("data") or {}).get("data") or []
            if room_list and isinstance(room_list[0], dict):
                room = room_list[0]
        if not room:
            logger.warning(f"[{self.name}] 抖音直播间信息获取失败，跳过解析 {url}")
            raise SkipParse("抖音直播间信息获取失败")

        status = room.get("status")  # 2=直播中
        ...
        for cover_url in cover.get("url_list") or []:
            if isinstance(cover_url, str) and cover_url.startswith("http"):
                images.append(cover_url)
                break
        headers = self._build_result_headers(DOUYIN_USER_AGENT)

        video_urls: List[List[str]] = []
        if self.live_clip_seconds > 0 and status == 2:
            flv_url = self._extract_live_flv_url(room.get("stream_url") or {})
            if flv_url:
    ...
    async def parse(
        ...
        async with self.semaphore:
            ...
            if self.live_parse and self._is_douyin_live_url(redirected_url):
                logger.debug(f"[{self.name}] parse: 检测到直播链接，使用直播解析")
                return await self._parse_douyin_live(session, redirected_url)
            ...
            music_url = result.get("music_url", "") if self.send_bgm else ""
            music_title = result.get("music_title", "") if self.send_bgm else ""
            hot_comments = []
            if self.hot_comment_count > 0:
                item_id = self._extract_douyin_item_id(redirected_url, url)
                if item_id:
                    hot_comments = await self.web_client.fetch_comments(
                        session, item_id, self.hot_comment_count
                    )
            ...
            if is_gallery and not video_url_lists:
                ...
                if hot_comments:
                    gallery_result["hot_comments"] = hot_comments
            ...
            if hot_comments:
                parsed_result["hot_comments"] = hot_comments
            logger.debug(
