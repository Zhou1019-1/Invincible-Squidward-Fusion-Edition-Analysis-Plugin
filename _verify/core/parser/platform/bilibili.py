...
ONLINE_TOTAL_API = "https://api.bilibili.com/x/player/online/total"
SUMMARY_API = "https://api.bilibili.com/x/web-interface/view/conclusion/get"
LIVE_ROOM_INFO_API = "https://api.live.bilibili.com/room/v1/Room/get_info"
LIVE_PLAY_URL_API = "https://api.live.bilibili.com/room/v1/Room/playUrl"
CV_PATH_RE = re.compile(r"/read/cv(\d+)", re.IGNORECASE)
CV_QS_RE = re.compile(r"(?:^|[?&])id=(\d+)", re.IGNORECASE)
LIVE_ROOM_RE = re.compile(r"^/(\d+)(?:/|$)")

# ── CDN 优选（移植自 rconsole-plugin）────────────────────
# 慢速 CDN 特征 / 快速 CDN 特征 / P2P 域名镜像替换表
_SLOW_CDN_PATTERNS = (".mcdn.bilivideo.cn", "mountaintoys.cn", ".szbdyd.com")
_FAST_CDN_RE = re.compile(
    r"^(cn-[a-z]+-[a-z]+-\d+-\d+\.bilivideo\.com|upos-sz-mirror.*\.bilivideo\.com"
    r"|upos-hz-mirror.*\.bilivideo\.com)$"
)
_P2P_REPLACERS = [
    (
        re.compile(
            r"upos-sz-mirror08[ch]?\.bilivideo\.com|upos-hz-mirrorakam\.akamaized\.net"
        ),
        "upos-sz-mirrorhwo1.bilivideo.com",
    ),
    (
        re.compile(
            r"upos-sz-estgoss\.bilivideo\.com|upos-sz-mirrorali(ov|b)?\.bilivideo\.com"
        ),
        "upos-sz-mirroralio1.bilivideo.com",
    ),
    (
        re.compile(r".*\.mcdn\.bilivideo\.cn|cn(-[a-z]+){2}(-\d{2}){2}\.bilivideo\.com"),
        "upos-sz-mirrorcoso1.bilivideo.com",
    ),
]

# ── 编码偏好（移植自 rconsole-plugin）────────────────────
_CODEC_DETECT = (("av01", "av1"), ("hev1", "hevc"), ("avc1", "avc"))
_CODEC_PRIORITY = {
    "auto": {"av1": 1, "hevc": 2, "avc": 3},
    "av1": {"av1": 1, "hevc": 2, "avc": 3},
    "hevc": {"hevc": 1, "av1": 2, "avc": 3},
    "avc": {"avc": 1, "av1": 2, "hevc": 3},
}
VALID_CDN_MODES = {0, 1, 2}


def _replace_p2p_url(url: str) -> str:
    """将慢速/P2P CDN 域名替换为官方镜像域名。"""
    try:
        parsed = urlparse(url)
    except (AttributeError, TypeError, ValueError):
        return url
    host = parsed.netloc
    for pattern, target in _P2P_REPLACERS:
        if pattern.search(host):
            # .szbdyd.com 使用 query 中携带的 xy_usource 作为替换目标
            if host.endswith(".szbdyd.com"):
                source = parse_qs(parsed.query).get("xy_usource", [None])[0]
                if source:
                    target = source
...
def select_cdn_url(base_url: str, backup_urls: Optional[List[str]] = None,
                   cdn_mode: int = 0) -> str:
    """CDN 优选：自动避开慢速 CDN。

    Args:
        base_url: 主播放地址
        backup_urls: 备用播放地址列表
        cdn_mode: 0=自动避慢，1=关闭（原样返回），2=P2P 域名替换

    Returns:
        优选后的播放地址
    """
    if cdn_mode == 1 or not base_url:
        return base_url
    if cdn_mode == 2:
        return _replace_p2p_url(base_url)
    ...
    def is_slow(u: str) -> bool:
        try:
            ...
            return False
        return any(p in host for p in _SLOW_CDN_PATTERNS)
    ...
    def is_fast(u: str) -> bool:
        try:
            ...
            return False
        return bool(_FAST_CDN_RE.match(host))

    if is_fast(base_url) or not is_slow(base_url):
        return base_url
    for url in backup_urls or []:
        if is_fast(url):
            return url
    for url in backup_urls or []:
        if not is_slow(url):
...
def _detect_codec(codecs: str) -> str:
    """从 DASH 流的 codecs 字符串识别编码族（av1/hevc/avc）。"""
    codecs = codecs or ""
    for key, name in _CODEC_DETECT:
        if key in codecs:
            return name
    return "avc"
MIXIN_KEY_ENC_TAB = [
...
class BilibiliParser(BaseVideoParser):
    ...
    def __init__(
        ...
        try:
            cdn_mode_value = int(cdn_mode)
            self.cdn_mode = cdn_mode_value if cdn_mode_value in VALID_CDN_MODES else 0
        except (TypeError, ValueError):
            self.cdn_mode = 0
        codec = str(video_codec or "auto").strip().lower()
        self.video_codec = codec if codec in _CODEC_PRIORITY else "auto"
        try:
            self.file_size_limit_mb = max(0, int(file_size_limit_mb))
        except (TypeError, ValueError):
            self.file_size_limit_mb = 0
        self.show_online = bool(show_online)
        self.ai_summary = bool(ai_summary)
        self.live_parse = bool(live_parse)
        try:
            self.live_clip_seconds = max(0, int(live_clip_seconds))
        except (TypeError, ValueError):
            self.live_clip_seconds = 0
    ...
    @staticmethod
    def _is_bilibili_live_url(url: str) -> bool:
        """判断是否为B站直播间链接（含房间号）。"""
        try:
            parsed = urlparse(url)
        except (AttributeError, TypeError, ValueError):
            return False
        host = (parsed.hostname or "").lower().rstrip(".")
        return host == "live.bilibili.com" and bool(LIVE_ROOM_RE.search(parsed.path))

    def _extract_cv_id(self, url: str) -> Optional[str]:
        """从URL中提取专栏ID（支持 read/cv 与 read/mobile?id= 两种形式）。"""
        try:
            parsed = urlparse(url)
        except (AttributeError, TypeError, ValueError):
            return None
        host = (parsed.hostname or "").lower().rstrip(".")
        if not _is_trusted_bilibili_host(host):
            return None
        match = CV_PATH_RE.search(parsed.path)
        if match:
            return match.group(1)
        if parsed.path.rstrip("/").endswith("/read/mobile"):
            match = CV_QS_RE.search(url)
            if match:
                return match.group(1)
        return None
    ...
    async def parse_article(
        ...
        """解析B站专栏文章，提取标题/作者与全部原图（移植自 rconsole-plugin）。
        ...
        Raises:
            RuntimeError: API返回错误或专栏无有效内容时
        """
        headers = self._build_api_headers(
            referer=page_url, cookie_header=cookie_header
        )
        async with session.get(
            ARTICLE_INFO_API,
            params={"id": cv_id},
        ...
        images = [
            u
            for u in (data.get("origin_image_urls") or [])
            if isinstance(u, str) and u.startswith("http")
        ]
        if not title and not images:
            raise RuntimeError(f"专栏无有效内容: {page_url}")
    ...
    async def parse_live(
        ...
        """解析B站直播间信息，可选录制直播片段（移植自 rconsole-plugin）。
        ...
        Returns:
            标准化元数据字典；开启片段录制且直播中时附带
            liveclip: 协议前缀的下载地址

        Raises:
            RuntimeError: 链接无效或直播间信息获取失败时
        """
        parsed = urlparse(page_url)
        match = LIVE_ROOM_RE.search(parsed.path)
        if not match:
            raise RuntimeError(f"无法解析直播间链接: {page_url}")
        ...
        async with session.get(
            ...
            j = await self._check_json_response(resp)
        await self._handle_api_response(j, "live room info")
        data = j.get("data") or {}
        if not data:
            raise RuntimeError(f"直播间信息获取失败: {page_url}")

        title = data.get("title") or ""
        desc_lines = []
        description = re.sub(r"</?p>", "", data.get("description") or "").strip()
        if description:
            desc_lines.append(description)
        if data.get("tags"):
            desc_lines.append(f"标签：{data['tags']}")
        area = f"{data.get('parent_area_name', '')}-{data.get('area_name', '')}".strip("-")
        if area:
            desc_lines.append(f"分区：{area}")
        if data.get("live_time"):
            desc_lines.append(f"直播时间：{data['live_time']}")
        if data.get("live_status") != 1:
            desc_lines.append("当前未在直播")

        images = [
            u
            for u in (data.get("user_cover"), data.get("keyframe"))
            if isinstance(u, str) and u.startswith("http")
        ]
        image_headers, video_headers = self._build_media_headers(
            referer=page_url,
        ...
        if self.live_clip_seconds > 0 and data.get("live_status") == 1:
            flv_url = await self._get_live_flv_url(room_id, session, cookie_header)
            if flv_url:
                video_urls = [[f"liveclip:{self.live_clip_seconds}||{flv_url}"]]
    ...
    async def _get_live_flv_url(
        self,
        room_id: str,
        session: aiohttp.ClientSession,
        cookie_header: str = "",
    ) -> Optional[str]:
        """获取直播间 FLV 流地址，失败时返回None。"""
        try:
            ...
            durls = ((j or {}).get("data") or {}).get("durl") or []
            if durls:
                return durls[0].get("url")
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.warning(f"[{self.name}] 直播流地址获取失败: {e}")
        return None
    ...
    async def get_online_total(
        ...
        """获取视频实时在线人数（移植自 rconsole-plugin），失败时返回None。

        Returns:
            (总人数, 网页端人数) 元组，失败时为None
        """
        try:
            headers = self._build_api_headers(cookie_header=cookie_header)
            async with session.get(
            ...
            if total is None:
                return None
            return total, data.get("count")
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.warning(f"[{self.name}] 在线人数获取失败: {e}")
            return None
    ...
    async def get_ai_summary(
        ...
        """获取B站官方AI视频总结（WBI签名，需要Cookie），失败或无总结时返回None。

        移植自 rconsole-plugin 的 view/conclusion/get 流程。
        """
        ...
        try:
            ...
            async with session.get(
                ...
                j = await resp.json(content_type=None)
            model = ((j or {}).get("data") or {}).get("model_result") or {}
            summary = str(model.get("summary", "") or "").strip()
            if not summary:
                return None
            lines = ["AI 视频总结：", summary]
            for outline in model.get("outline") or []:
                outline_title = str(outline.get("title", "") or "").strip()
                if outline_title:
                    lines.append(f"◆ {outline_title}")
                for part in outline.get("part_outline") or []:
                    timestamp_text = format_duration_ms(
                        (part.get("timestamp", 0) or 0) * 1000
                    )
                    content = str(part.get("content", "") or "").strip()
                    if content:
                        lines.append(f"  {timestamp_text} {content}")
            return "\n".join(lines)
        except (aiohttp.ClientError, asyncio.TimeoutError, RuntimeError) as e:
            logger.warning(f"[{self.name}] AI视频总结获取失败: {e}")
            return None

    async def _attach_extra_info_to_result(
        self,
        session: aiohttp.ClientSession,
        result: Dict[str, Any],
        bvid: Optional[str],
        ...
        """按配置为UGC结果附加在线人数与AI视频总结（移植自 rconsole-plugin）。"""
        if not bvid or not cid:
            return
        extra_lines = []
        if self.show_online:
            online = await self.get_online_total(bvid, cid, session, cookie_header)
            if online:
                total, count = online
                extra_lines.append(
                    f"当前有 {total} 人在观看，其中 {count} 人在网页端观看"
                )
        if self.ai_summary:
            summary = await self.get_ai_summary(
                bvid,
                cid,
                info.get("owner_mid"),
                session,
    ...
    def can_parse(self, url: str) -> bool:
        ...
        if hostname == "live.bilibili.com":
            if self.live_parse and LIVE_ROOM_RE.search(parsed.path):
                logger.debug(f"[{self.name}] can_parse: 匹配直播链接 {url}")
                return True
    ...
    def extract_links(self, text: str) -> List[str]:
        ...
        cv_pattern = (
            r"https?://(?:www|m|mobile)\.bilibili\.com/read/cv"
            r'(\d+)[^\s<>"\'()]*'
        )
        for match in re.finditer(cv_pattern, text, re.IGNORECASE):
            cv_id = match.group(1)
            cv_key = f"CV:{cv_id}"
            if cv_key not in seen_ids:
                seen_ids.add(cv_key)
                result_links_set.add(f"https://www.bilibili.com/read/cv{cv_id}")

        if self.live_parse:
            live_pattern = r'https?://live\.bilibili\.com/(\d+)[^\s<>"\'()]*'
            for match in re.finditer(live_pattern, text, re.IGNORECASE):
                room_id = match.group(1)
                live_key = f"LIVE:{room_id}"
                if live_key not in seen_ids:
                    seen_ids.add(live_key)
                    result_links_set.add(f"https://live.bilibili.com/{room_id}")
    ...
    def pick_best_video(self, dash_obj: Dict[str, Any]):
        ...
        按画质降序排列，同画质内按编码偏好与带宽排序（移植自 rconsole-plugin）。
        ...
        Returns:
            最佳视频流数据，未找到时为None
        """
        vids = self._quality_limited_videos(dash_obj)
        if not vids:
            return None
        codec_pref = _CODEC_PRIORITY.get(self.video_codec, _CODEC_PRIORITY["auto"])
        return sorted(
            vids,
            key=lambda x: (
                -(x.get("id", 0) or 0),
                codec_pref.get(_detect_codec(x.get("codecs")), 9),
                -(x.get("bandwidth", 0) or 0),
            ),
        )[0]

    def _quality_limited_videos(self, dash_obj: Dict[str, Any]) -> List[Dict[str, Any]]:
        """返回按 max_qn 上限过滤后的候选视频流列表。"""
    ...
    @staticmethod
    def _estimate_size_mb(video_bw: float, audio_bw: float, duration_s: float) -> float:
        """按码率与时长估算音视频合并后的文件大小（MB）。"""
        return (video_bw + audio_bw) / 8 * duration_s / 1024 / 1024

    def _pick_video_with_budget(
        self, dash_obj: Dict[str, Any], duration_ms: Optional[int] = None
    ):
        """在文件大小预算内选择视频流（移植自 rconsole-plugin 的降档策略）。

        候选按画质降序、编码偏好、带宽降序排列，选取第一个估算大小
        不超过 file_size_limit_mb 的流；全部超出时回退最低带宽流。

        Args:
            dash_obj: DASH格式视频数据
            duration_ms: 视频时长（毫秒），未知时跳过预算降档
        ...
        if self.file_size_limit_mb <= 0 or not duration_ms:
            return ordered[0]
        audios = dash_obj.get("audio") or []
        audio_bw = (audios[0].get("bandwidth", 0) or 0) if audios else 0
        duration_s = max(0.0, duration_ms / 1000)
        for video in ordered:
            estimated = self._estimate_size_mb(
                video.get("bandwidth", 0) or 0, audio_bw, duration_s
            )
            if estimated <= self.file_size_limit_mb:
                return video
        logger.debug(
            f"[{self.name}] 所有画质估算大小均超过 {self.file_size_limit_mb}MB，"
            "已回退最低带宽流"
        )
        return min(ordered, key=lambda x: (x.get("bandwidth", 0) or 0))
    ...
    def _build_dash_download_url(
        self, dash_obj: Dict[str, Any], duration_ms: Optional[int] = None
    ) -> Optional[str]:
        """从 DASH 数据中构建下载 URL（优先 video+audio）。

        选流时应用编码偏好与文件大小预算降档，直链经 CDN 优选。

        Args:
            dash_obj: DASH格式视频数据
            duration_ms: 视频时长（毫秒），用于大小预算降档
        """
        best_video = self._pick_video_with_budget(dash_obj, duration_ms)
        if not best_video:
            return None

        video_url = select_cdn_url(
            best_video.get("baseUrl") or best_video.get("base_url"),
    ...
    async def _get_ugc_direct_url(
        ...
        if merged_payload.get("durl"):
            durl_item = merged_payload["durl"][0]
            return select_cdn_url(
                durl_item.get("url"),
                durl_item.get("backup_url") or [],
                self.cdn_mode,
            )
        if bvid:
        ...
        return self._build_dash_download_url(
            dash_payload.get("dash") or {},
            duration_ms=dash_payload.get("timelength"),
        )
    ...
    async def parse_bilibili_minimal(
        ...
        if self.live_parse and self._is_bilibili_live_url(page_url):
            logger.debug(
                f"[{self.name}] parse_bilibili_minimal: 检测到直播链接，使用直播解析 {page_url}"
            )
            return await self.parse_live(
                page_url, session, cookie_header=cookie_header
            )
        ...
        cv_id = self._extract_cv_id(page_url)
        if cv_id:
            logger.debug(
                f"[{self.name}] parse_bilibili_minimal: 检测到专栏链接，使用专栏解析 {page_url}"
            )
            return await self.parse_article(
                page_url, cv_id, session, cookie_header=cookie_header
            )
        ...
        result.update(self._access_fields_from_info(access_info))
        if vtype == "ugc":
            await self._attach_extra_info_to_result(
                session=session,
                result=result,
                bvid=bvid,
                cid=cid,
