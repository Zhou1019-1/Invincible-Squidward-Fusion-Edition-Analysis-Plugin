...
class DownloadManager:
    ...
    def __init__(
        ...
        # 开启后，超过大小限制的视频不再丢弃，而是下载后作为文件发送
        # （移植自 rconsole-plugin 的超限转群文件能力）
        self.oversize_as_file = bool(oversize_as_file)
    ...
    async def process_metadata(
        ...
        *,
        ...
        for idx, url_list in enumerate(video_urls):
            ...
            if not contains_stream and not direct_fallback_selected:
                ...
                if reason:
                    if (
                        "超过限制" in reason
                        and self.oversize_as_file
                        and self.cache_dir_available
                    ):
                        # 超限转文件：强制本地下载，后续作为文件发送
                        mode = "local"
                    else:
                        if "超过限制" in reason:
                            size_exceeded = True
        ...
        for result in download_results:
            ...
            if kind == "video":
                ...
                if (
                    ...
                    if self.oversize_as_file:
                        # 超限转文件：保留文件，标记为文件发送模式
                        video_modes[idx] = "file"
        ...
        valid_video_count = sum(
            1 for mode in video_modes if mode in ("local", "direct", "file")
        )
